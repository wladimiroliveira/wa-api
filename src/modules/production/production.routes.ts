...
export default async function productionRoutes(app: FastifyInstance) {
  ...
  r.post(
    "/productions",
    { preHandler: requirePermission(Permission.PRODUCTION_WRITE), schema: { body: createProductionSchema } },
    async (req, reply) => {
      try {
  ...
  r.get("/productions", { preHandler: requirePermission(Permission.PRODUCTION_READ) }, async () => listProductions());

  r.get(
    "/productions/:id",
    { preHandler: requirePermission(Permission.PRODUCTION_READ), schema: { params: productionIdParamSchema } },
    async (req, reply) => {
